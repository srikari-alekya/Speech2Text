...
btnConvert=tk.Button(MainFrame, font=('arial',30,'bold'),text="Convert to Text",width=18,height=2,command=speech2text)
...
def speech2text():
    ...
    with sr.Microphone() as source:
        ...
        root.update()  # Update the UI to show "Listening..." message
        try:
            audio = r.listen(source)
            text = r.recognize_google(audio)
            txtSpeech.insert(tk.END, text + "\n")
